...
class AccountInvoice(models.Model):
    ...
    uuid_fel = fields.Char(string='No. Factura', readonly=True, default=0, copy=False, states={'draft': [('readonly', False)]}, help='UUID returned by certifier')  # No. Invoice
    fel_serie = fields.Char(string='Serie', readonly=True, states={'draft': [('readonly', False)]}, copy=False, help='Raw Serial number return by GFACE or FEL provider')  # Fel Series
    fel_no = fields.Char(string='Numero.', readonly=True, states={'draft': [('readonly', False)]}, copy=False, help='Raw Serial number return by GFACE or FEL provider')
    fel_date = fields.Char(string='Fecha DTE.', readonly=True, states={'draft': [('readonly', False)]}, copy=False, help='Raw date return by GFACE or FEL provider')
    fel_received_sat = fields.Char(string='Acuse Recibo SAT', readonly=True, states={'draft': [('readonly', False)]}, copy=False)
    uuid = fields.Char(string='UUID', readonly=True, states={'draft': [('readonly', False)]}, copy=False, help='UUID given to the certifier to register the document')
    no_acceso = fields.Char(string='Numero de Acceso', readonly=True, states={'draft': [('readonly', False)]}, copy=False, help='Electronic singnature given sent to FEL')  # Access Number
    ...
    partner_street = fields.Char('Direccion', related="partner_id.street", readonly=False)
    restrict_mode_hash_table = fields.Boolean(default=False)
    not_use_tax = fields.Boolean('Ncre sin impuestos', default=False)
    ...
    def generate_xml(self):
        ...
        for inv_line in self.invoice_line_ids:
            ...
            if inv_line.tax_ids:
                ...
                grabable = round((total_price - discount), 2)
    ...
    def get_direccion(self, xml_str):
        ...
        if xml_str:
            ...
            #root = tree.getroot()
            for child in tree:
                if child.tag == 'direcciones': #---> DireccionReceptor
                    for subchild in child:
                        if subchild.tag == 'direccion': #---> Direccion
                            var = subchild.text
        return var
    ...
    def get_nombre(self, xml_str):
        ...
        tree = fromstring(xml_str)
        for child in tree:
            if child.tag == 'nombre': #---> NombreReceptor
                var = child.text
        return var
    ...
    def action_print_fel(self):
        ...
        #     'target': 'self',
        # }
        return self.env.ref('account_invoice_megaprint.xetechs_fel_format').report_action(self)

    def action_print_fel_exportacion(self):
        return self.env.ref('account_invoice_megaprint.xetechs_fel_exportacion_format').report_action(self)
