...
class MailActivity(models.Model):
    ...
    required_user_id = fields.Many2one('res.users', string='Solicitado por', index=True, tracking=True, default=lambda self: self.env.user)
    res_model_id = fields.Many2one('ir.model', 'Document Model', index=True, ondelete='cascade', required=True, default=lambda self: self.env.ref('todo_list.model_activity_general'))
    res_id = fields.Many2oneReference(string='Related Document ID', index=True, required=True, model_field='res_model', default=lambda self: self.env.ref('todo_list.general_activities'))
    priority = fields.Selection([('0', 'Normal'), ('1', 'Important'), ('2', 'Very Important'), ('3', 'Urgent')], default='0', index=True, store=True)
    recurring = fields.Boolean(string="Recurring", store=True)
    state = fields.Selection([('today', 'Today'), ('planned', 'Planned'), ('done', 'Done'), ('overdue', 'Expired'), ('cancel', 'Cancelled'), ], 'State', compute='_compute_state', store=True)
    interval = fields.Selection([('Daily', 'Daily'), ('Weekly', 'Weekly'), ('Monthly', 'Monthly'), ('Quarterly', 'Quarterly'), ('Yearly', 'Yearly')], string='Recurring Interval')
    new_date = fields.Date(string="Next Due Date", store=True)
    tag_ids = fields.Many2many('todo.tag', 'todo_tags', 'tag_id', 'id', string='Etiquetas', help="Clasifique sus to do con etiquetas personalizadas.")
