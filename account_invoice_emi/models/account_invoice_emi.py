...
class Account_Invoice_EMI(models.Model):
    ...
    name = fields.Char(string='EMI Number', required=True, copy=False, readonly=True, index=True, default=lambda self: _('New'), tracking=True)
    title = fields.Char(string='Title EMI', required=False, readonly=False)
    so_id = fields.Many2one('sale.order', string="Sales Order", required=True, domain="[('state', '=', 'done')]", tracking=True)
    company_id = fields.Many2one('res.company', string='Company', default=lambda self: self.env.company)
    type = fields.Selection([('fixed', 'Fixed'), ('manual', 'Manual')], string="EMI Type", default="manual", tracking=True)
    total = fields.Integer(string="Total EMI", tracking=True, default=1)
    total_emi = fields.Integer(string="Total EMI", readonly=True, tracking=True, compute="_compute_emi_invoices")
    paid_total = fields.Integer(string="Invoiced EMI", readonly=True, compute="_compute_emi_invoices")
    interest = fields.Float(string="Interest Rate", tracking=True)
    ...
    state = fields.Selection([('draft', 'Draft'), ('confirm', 'Confirm'), ('to_approved', 'To Be Approved'), ('approved', 'Approved'), ('done', 'Done'), ('reject', 'Reject')], string="State", default="draft", select=True, readonly=True, copy=False, tracking=True)
    ...
    total_invoice = fields.Integer(string="Total Invoice", compute="get_total_invoice", tracking=True)
    emi_amount = fields.Monetary(string="EMI Amount", compute="get_total_invoice", store=True, tracking=True)
    interest_amount = fields.Monetary(string="EMI Interest", compute="get_total_invoice", store=True, tracking=True)
    total_amount = fields.Monetary(string="Total Amount", compute="get_total_invoice", store=True, tracking=True)
    journal_id = fields.Many2one('account.journal', string="Invoice Journal", tracking=True)
    start_date = fields.Date(string="Start Date", tracking=True)
    emi_tax_ids = fields.Many2many('account.tax', 'emi_line_tax', 'emi_line_id', 'tax_id', string='Taxes', domain=[('type_tax_use', '!=', 'none'), '|', ('active', '=', False), ('active', '=', True)], default=[2])
...
class Account_Invoice_EMI_Line(models.Model):
    ...
    state = fields.Selection([('draft', 'Draft'), ('to_invoice', 'To Invoice'), ('invoiced', 'Invoiced')], string="State", default="draft")
